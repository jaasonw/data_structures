// TODO
// big 3 (be extra careful about destructor and copy constructor)
// delete
// 
...
void BTree<T>::copy_tree(const BTree<T>& other) {
    ...
    for (size_t i = 0; i < other.subset_size; i++) {
        if (other.subset[i] != nullptr) {
            subset[i] = new BTree<T>(other.duplicates_allowed);
...
template <typename T>
void BTree<T>::fix_shortage(int index) {
    BTree<T>* left = subset[index];
    BTree<T>* right = subset[index + 1];
